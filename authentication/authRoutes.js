...
function validUserData(user_data){
    const hasEmail = typeof user_data.email == 'string' && user_data.email.trim() != ""
    const hasUsername = typeof user_data.username == 'string' && user_data.username.trim() != ""
    const hasName = typeof user_data.username == 'string' && user_data.username.trim() != ""
...
router
    ...
    .post(async (req, res)=>{
        try{
            ...
            res.json({error: "Invalid", status: 404})
        }
    })
